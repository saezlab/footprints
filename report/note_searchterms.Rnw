...
ae2pmid = function(args) {
    ...
    one_accession = function(acc) {
        ...
        doi = unique(unlist(lapply(XML::xpathSApply(bib, q), XML::xmlToList)))
        knitcitations::citep(doi)
    }

    lapply(args, function(x) { #TODO: maybe simplify this w/ numerical citations
        cite = one_accession(x)
