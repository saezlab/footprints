...
#mut = mut[,c("TP53", "VHL", "KRAS", "NRAS", "EGFR", "PIK3CA", "STK11")]

scores = c('gsea_reactome', 'gsea_go', 'speed_matrix', 'pathifier', 'spia', 'pathifier') %>%
...
```{r capture, echo=FALSE, fig.width=4}
keep = result %>%
    group_by(m) %>%
...
n_methods = length(unique(result$method))
...
```{r nras, echo=FALSE, fig.width=4}
cur = filter(result, m == "NRAS")
lmax = max(abs(cur$statistic))
p1 = cur %>%
    mutate(label = ifelse(adj.p < 0.2, "*", "")) %>%
    mutate(label = ifelse(adj.p < 0.1, "***", label)) %>%
    plt$matrix(statistic ~ s + method, color="estimate", limits=c(-lmax,lmax))
print(p1)
```
...
```{r egfr, echo=FALSE, fig.width=4}
cur = filter(result, m == "EGFR")
lmax = max(abs(cur$statistic))
p1 = cur %>%
    mutate(label = ifelse(adj.p < 0.2, "*", "")) %>%
    mutate(label = ifelse(adj.p < 0.1, "***", label)) %>%
...
```{r pik3ca, echo=FALSE, fig.width=4}
cur = filter(result, m == "PIK3CA")
lmax = max(abs(cur$statistic))
p1 = cur %>%
    mutate(label = ifelse(adj.p < 0.05, "*", "")) %>%
    mutate(label = ifelse(adj.p < 1e-3, "***", label)) %>%
