...
do$plot_noclip(do$p + theme(strip.text.x = element_text(size = 11)))
@
\caption{Relative pathway activation patterns for activity measurement vs.
inference by PROGENy. Data sources are experiments where PTMs (mostly Western
...
Difference quantified using a one-tailed t-test.
}
\label{fig:sig-val}
\end{figure}
