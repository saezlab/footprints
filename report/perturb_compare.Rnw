...
\caption{Distribution of pathway scores for individual experiments (right) and
    their statistics (left). Right: perturbation experiments on horizontal
        ...
        (teal) or inhibiting (orange). Total magnitude of pathway activation
        scaled for each experiment. Left: statistical associations between
...
shwon in figure 2. It does, however, show that competing pathway methods are
