...
the combination between a mutation/pathway and the number of cell lines in each
subset. Subsets are defined by: (1) Whether a mutation is present (mut) or not (wt),
or disregarding mutational status (blank). (2) Whether the pathway score is in the
top (active) or bottom quartile (inactive), or neither (blank) of the subset
