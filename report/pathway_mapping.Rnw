...
\begin{table}[H]
\centering
\caption{Pathway mapping for Gene Ontology. GO categories (ID and name) shown
...
\begin{table}[H]
\centering
\caption{Pathway mapping for Reactome and Pathifier. Listed are reactome
...
\begin{table}[H]
\centering
\caption{Pathway mapping for SPIA. KEGG pathway IDs and names that SPIA relies
...
\begin{table}[H]
\centering
\caption{Pathway mapping for PARADIGM. This method is different as it performs
...
\begin{table}[H]
\centering
\caption{Pathway mapping for the signatures published and validated in Gatza et al. (2009)}
\begin{tabular}{cc}
\hline
